import { FileText, UploadCloud } from "lucide-react";

export function ProfileResumeSection() {
  return (
    <section className="rounded-xl border border-border bg-surface p-8 shadow-card">
      <div>
        <h2 className="text-[22px] font-bold leading-7 text-text-primary">
          Resume
        </h2>
        <p className="mt-2 text-sm font-semibold leading-5 text-text-muted">
          Upload an existing resume to auto-fill the profile, or generate a new
          tailored one from your details below.
        </p>
      </div>

      <div className="mt-8 flex min-h-62 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface-secondary px-6 py-10 text-center">
        <div className="flex size-14 items-center justify-center rounded-full border border-border bg-surface shadow-card">
          <UploadCloud className="size-6 text-accent" aria-hidden="true" />
        </div>
        <p className="mt-5 text-base font-bold leading-6 text-text-primary">
          Click to upload or drag and drop
        </p>
        <p className="mt-1 text-sm font-medium leading-5 text-text-secondary">
          PDF formatting only. Maximum file size 5MB.
        </p>
        <button
          type="button"
          className="mt-6 rounded-md border border-border bg-surface px-6 py-3 text-sm font-bold text-text-primary shadow-card transition-colors hover:bg-surface-secondary"
        >
          Select Resume
        </button>
      </div>

      <div className="mt-8 flex items-center justify-between border-t border-border pt-8">
        <p className="text-sm font-semibold leading-5 text-text-muted">
          Need a fresh document based on the fields below?
        </p>
        <button
          type="button"
          className="inline-flex items-center gap-2 rounded-md bg-accent px-5 py-3 text-sm font-bold text-accent-foreground shadow-card transition-colors hover:bg-accent-dark"
        >
          <FileText className="size-4" aria-hidden="true" />
          Generate Resume from Profile
        </button>
      </div>
    </section>
  );
}
